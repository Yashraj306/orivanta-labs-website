...
Orivanta Labs — a premium agency website for a digital growth agency serving restaurants and local businesses. Built with TanStack Start and deployed on Netlify.

## Pages

| Route | Component | Purpose |
|---|---|---|
| `/` | `routes/index.tsx` | Home — 7 sections: Hero, Services, How It Works, Before & After, Why Us, Testimonials, Contact |
| `/services` | `routes/services.tsx` | Detailed breakdown of all 6 services |
| `/portfolio` | `routes/portfolio.tsx` | Case studies including featured Ajora Ethiopian Kitchen project |
| `/about` | `routes/about.tsx` | Company story, values, team |
| `/contact` | `routes/contact.tsx` | Full contact form with free audit request |

## Shared Components

- `src/components/Navbar.tsx` — Fixed top nav, scrolled state, mobile hamburger
- `src/components/Footer.tsx` — Site-wide footer with service links

## Forms

Contact forms use **Netlify Forms** via AJAX:
1. `public/__forms.html` — static skeleton for Netlify build-time detection
2. React submits to `fetch('/__forms.html', { method: 'POST' })` — not `/`
3. Form `name="contact"` must match between skeleton and React component

## Design System

- Primary: `#004B49` (dark teal-green)
- Accent: `#00D4C8` (cyan)
- Alternate bg: `#F8F6F1` (warm cream)
- Sections alternate: white → cream → brand → white
- All page heroes: `bg-[#004B49]` with white/cyan text

## Non-Obvious Decisions

- `__root.tsx` uses `shellComponent` (not `component`) — TanStack Start SSR shell pattern
- `routeTree.gen.ts` is auto-generated — never edit manually
- Netlify Forms enable script was run: `node /opt/buildhome/.claude/skills/netlify-forms/scripts/enable.cjs`
